...
def _home_context(user: User) -> dict[str, list]:
...
def _save_series_opinion(user: User, show: dict, viewed: bool, opinion_value: str | None) -> None:
    opinion_enum = None
    if opinion_value:
        try:
...
def series_detail(serie_id: int):
    ...
    context = _series_context(serie_id, g.user)

    if request.method == "GET" and not context["current_viewed"]:
        _save_series_opinion(g.user, context["show"], True, context["current_opinion"])
        context = _series_context(serie_id, g.user)

    if request.method == "POST":
        opinion_value = str(request.form.get("opinion") or "").strip() or None
        _save_series_opinion(g.user, context["show"], True, opinion_value)
